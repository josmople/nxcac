...
def build_matcher(matcher="dynamic_similarity_matcher", hidden_dim=256, matcher_proj_dim=256, dynamic_proj_dim=128, use_bias=True):
    matcher_name = matcher
    if matcher_name == 'inner_product_matcher':
        return InnerProductMatcher()
    elif matcher_name == 'bilinear_similarity_matcher':
        return BilinearSimilarityMatcher(hidden_dim=hidden_dim,
                                         proj_dim=matcher_proj_dim,
                                         use_bias=use_bias)
    elif matcher_name == 'dynamic_similarity_matcher':
        return DynamicSimilarityMatcher(hidden_dim=hidden_dim,
                                        proj_dim=matcher_proj_dim,
